...
class TestMeetupInfoFetch(TestCase):
  ...
  def setUp(self):
    @mock.patch.object(MeetupApiLookupAgent, 'get_response')
    # @mock.patch(MeetupApiLookupAgent)
    mock_get_response.json.return_value = False
    self.lookup_agent = mock_meetup_api_lookup_agent

  def test_returns_meetup_collection(self):
    meetup = Meetup(name="Meeetup1", meetup_link='http://www.meetup.com/papers-we-love/')
    i_fetch = MeetupInfoFetch([meetup], self.lookup_agent)
